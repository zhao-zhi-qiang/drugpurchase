//控制层
app.controller('caseManagementController', function ($scope, $controller, caseManagementService, toaster) {
    ...
    $scope.search = function (page, rows) {
        caseManagementService.search(page, rows, $scope.searchEntity).success(
            function (response) {
    ...
    $scope.findOne = function (id) {
        caseManagementService.findOne(id).success(
            function (response) {
    ...
    $scope.save = function () {
        ...
        caseManagementService.update($scope.entity).success(
            function (response) {
                if (response.success) {
                    $scope.pop();
                    $("#bt1").click();
    ...
    $scope.yanshi = function (id) {
        caseManagementService.yanshi(id).success(function (response) {
            if (response.success) {
