...
function tiaozhuan(pages){
	$.post("http://localhost:80/mysocial/dailyAuditSelectAll",{"pages":pages,"approvalStatus":0,"typeStatus":2},function(data){
		console.log(data);
		fenye(data);
	})
}
...
function upload(){
	...
	var id = $("#id1").val();
				if (upload){
					...
					$.ajax({
						type:"post",
						url:"http://localhost:80/mysocial/specialAuditUploadFile",
						data:formData,
						processData : false,
						contentType : false,
